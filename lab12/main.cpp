...
void secondPass(GLFWwindow* window) {
    ...
    // TODO #3B
    //COMMENT TO SHOW THIS IS MINE
    glm::mat4 orthoProj = glm::ortho(-1, 1, -1, 1);
    postprocessingShaderProgram->useProgram();
    glUniformMatrix4fv(postprocessingShaderProgramUniforms.projectionMtx, 1, GL_FALSE, &orthoProj[0][0]);
    glBindTexture(GL_TEXTURE_2D, fboTextureHandle);
    glBindVertexArray( vaos[VAOS.TEXTURED_QUAD] );
    glDrawElements(GL_TRIANGLE_STRIP, 4, GL_UNSIGNED_SHORT, (void*)0);
}
